...
export default function ConversationsList({
  ...
  // Debug logging for Ably connection
  useEffect(() => {
    if (__DEV__) {
      console.log('[DEBUG] Ably key exists:', !!process.env.EXPO_PUBLIC_ABLY_API_KEY);
      console.log('[DEBUG] User ghlUserId:', user?.ghlUserId);
      console.log('[DEBUG] User userId:', user?.userId);
      console.log('[DEBUG] Location ID:', user?.locationId);
  ...
  useEffect(() => {
    // Try ghlUserId first (from backend), fall back to userId
    const userIdForAbly = user?.ghlUserId || user?.userId;
    
    if (!userIdForAbly || !locationId || !contactObjectId) {
      console.log('[Ably] Missing required data:', {
        userIdForAbly,
        locationId,
        contactObjectId
      });
    ...
    console.log('[Ably] Setting up real-time connection with user ID:', userIdForAbly);
    
    // Check if API key exists
    if (!process.env.EXPO_PUBLIC_ABLY_API_KEY) {
      console.error('[Ably] ❌ Missing EXPO_PUBLIC_ABLY_API_KEY environment variable!');
      return;
    }
    
    const ably = new Ably.Realtime({
      key: process.env.EXPO_PUBLIC_ABLY_API_KEY,
      clientId: userIdForAbly,
      logLevel: __DEV__ ? 4 : 0, // Verbose logging in dev
      echoMessages: false, // Don't echo our own messages
    });

    ablyRef.current = ably;

    const channelName = `user:${userIdForAbly}`;
  ...
  return (
    ...
    >
      ...
      {/* Connection indicator - only show when truly disconnected */}
      {!isRealtimeConnected && !loadingMessages && (
