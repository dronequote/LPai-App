...
interface Props {
 visible: boolean;
 onClose: () => void;
 onSubmit?: (contact: any) => void; // Now optional, receives created contact
 isModal?: boolean;
}
...
export default function AddContactForm({
 ...
 // Handle submit - NOW USING CONTACT SERVICE
 const handleSubmit = async () => {
   if (!validateForm()) return;
   
   if (!user?.locationId) {
     Alert.alert('Error', 'Unable to create contact. Please try logging in again.');
     return;
   }
   ...
   const contactData = {
     ...
     locationId: user.locationId, // Include locationId
   };
   
   try {
     if (__DEV__) {
       console.log('[AddContactForm] Creating contact with data:', contactData);
     }
     
     // Use contactService to create the contact
     const newContact = await contactService.create(contactData);
     
     if (__DEV__) {
       console.log('[AddContactForm] Contact created successfully:', newContact);
     }
     
     // Call onSubmit with the created contact (if provided)
     if (onSubmit) {
  ...
  console.error('[AddContactForm] Error creating contact:', error);
  
  // Log the full error response for debugging
  if (error.response) {
    console.error('[AddContactForm] Error response status:', error.response.status);
    console.error('[AddContactForm] Error response data:', error.response.data);
  }
       ...
       <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
         ...
         <View style={styles.section}>
           ...
           <TextInput
             style={[
               styles.input,
               companyName.length > 0 ? styles.inputValid : null
             ]}
             value={companyName}
