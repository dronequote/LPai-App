...
class SMSService extends BaseService {
 ...
 async send(
   ...
   const payload = {
     ...data,
     contactId: data.contactObjectId, // Backend expects 'contactId' (MongoDB _id as string)
   };
   delete payload.contactObjectId;
 ...
 async sendSimple(params: SendSmsParams): Promise<SMSResponse> {
   const { contactObjectId, message, templateKey = 'custom', fromNumber } = params;  // Changed from contactId
 ...
 async sendBatch(
   ...
   // Map to what backend expects
   const payload = {
     contactIds: contactObjectIds,  // Backend expects 'contactIds'
     message,
     templateKey,
     fromNumber,
     locationId: user.locationId,
   ...
   // Map response back to our field names
   if (result.results) {
     result.results = result.results.map((r: any) => ({
       ...r,
       contactObjectId: r.contactId || r.contactObjectId,
     }));
   }
 ...
 async getConversationHistory(
   ...
   // For now, using conversation endpoint - map to what backend expects
   const endpoint = `/api/conversations?locationId=${locationId}&contactId=${contactObjectId}&type=sms`;  // Backend expects contactId
