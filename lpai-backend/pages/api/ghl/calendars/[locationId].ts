...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // LOG the outgoing request to GHL
    const ghlReq = {
    ...
    console.log('[CALENDARS][API] About to call GHL with:', ghlReq);

    let ghlRes;
    try {
      ghlRes = await axios.get(ghlReq.url, {
        headers: ghlReq.headers,
        params: ghlReq.params,
      });
      console.log('[CALENDARS][API] Raw GHL response:', ghlRes.status, ghlRes.data);
    } catch (err: any) {
      console.error('[CALENDARS][API] Error from GHL:', err.response?.status, err.response?.data || err.message);
      throw err;
    }

    const calendars = ghlRes.data.calendars || [];
